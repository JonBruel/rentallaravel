...
@section('content')
    ...
    <div class="table">
        {!! Form::open(['action' => ['ContractController@limitedcontractupdate', $models[0]], 'class' => 'form-horizontal', 'autocomplete' => 'off']) !!}
        {!! Form::submit(__('Change number of guests'),['class' => "btn btn-primary col col-md-12", 'name' => 'Book', 'style' => 'opacity: 0.1']); !!}
        <br />
        <br />
        @if ($errors->any())
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function getWeeks(offset)
        {
            ...
            url = '/ajax/getweeks/' + houseid + '/' + culture + '/' + offset + '/' + periodid + '/' + contractid  + '/' + periodsshown ;
            new $.getJSON(url, function(periods)
            {
                ...
                periodchunk.forEach(periods => {
                    periods.forEach(period => {
                        ...
                        free = '<input class="" disabled="disabled"' + checked + ' name="checkedWeeks[' + period.id + ']" id="checkedWeeks_' + period.id + '" type="checkbox" value="'+period.id+'">';
                        if (period.committed && !period.chosen) {
                            // We don't show occupied periods
                            content += '';
                        }
                        ...
                        //content += '<tr' + style + '><td colspan="2">' + free + period.periodtext + '</td></tr>';
                            ...
                            content += '<tr' + style + '><td>' + period.periodtext + '</td><td>' + free + '</td><td><div id="addreduce_' + period.id + '" style="border: none; border-color: red">' + addreduce + '</div></td></tr>';
                        }
                    });
                });
                content += '</tr>';
